...
class DialogCalendar:
    ...
    async def start_calendar(
        ...
        for value in range(year - 2, year + 3):
            inline_row.append(
                InlineKeyboardButton(
                    text=str(value), callback_data=build_data("SET-YEAR", value, -1, -1)
                )
            )
        # nav buttons
        ...
        inline_row.append(
            InlineKeyboardButton(
                text="<<", callback_data=build_data("PREV-YEARS", year, -1, -1)
            )
        )
        inline_row.append(
            InlineKeyboardButton(
                text=">>", callback_data=build_data("NEXT-YEARS", year, -1, -1)
            )
        )
        inline_kb.append(inline_row)
    ...
    async def _get_month_kb(self, year: int):
        ...
        inline_row.append(
            InlineKeyboardButton(
                text=str(year), callback_data=build_data("START", year, -1, -1)
            )
        )
        inline_row.append(InlineKeyboardButton(text=" ", callback_data=ignore_callback))
        ...
        for week in month_calendar:
            ...
            for day in week:
                ...
                inline_row.append(
                    InlineKeyboardButton(
                        text=str(day),
                        callback_data=build_data("SET-DAY", year, month, day),
                    )
                )
            inline_kb.append(inline_row)
    ...
    async def process_selection(self, query: CallbackQuery) -> tuple:
        ...
        if data["act"] == "SET-MONTH":
            await query.message.edit_reply_markup(
                reply_markup=await self._get_days_kb(
